...
import com.happystays.cqrs.core.exceptions.HandlerException;
...
public class BookCommandDispatcher implements CommandDispatcher {
    ...
    public BaseResponse send(BaseCommand command) {
        ...
        if (Objects.isNull(handlers) || handlers.isEmpty()) {
            throw new HandlerException("No command handler was registered");
        }
        if (handlers.size() > 1) {
            throw new HandlerException("Cannot send command to more than one handler");
        }
